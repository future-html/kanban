...
const Home = ({ userId }) => {
    ...
    const handleDragStart = (e, dashboardId, oldBoardId, oldColumnId, taskId) => {
        // Pack up the task's current location data when we start dragging
        e.dataTransfer.setData("application/json", JSON.stringify({
            dashboardId, oldBoardId, oldColumnId, taskId
        }));
    };

    const handleDragOver = (e) => {
        e.preventDefault(); // This is required by browsers to allow dropping
    };

    const handleDrop = async (e, destDashboardId, newBoardId, newColumnId, newIndex) => {
        e.preventDefault();
        e.stopPropagation(); // Prevents double-firing if dropping on a task vs a column

        try {
            const data = JSON.parse(e.dataTransfer.getData("application/json"));
            const { dashboardId, oldBoardId, oldColumnId, taskId } = data;

            // Ensure we aren't trying to drag across entirely different dashboards
            if (dashboardId !== destDashboardId) {
                alert("Moving tasks between different dashboards is not supported.");
                return;
            }
            ...
            await taskService.moveTask(
                dashboardId, taskId,
                oldBoardId, oldColumnId,
                newBoardId, newColumnId,
                newIndex
            );

            fetchBoards(); // Refresh UI to show the new position
        } catch (err) {
                            ...
                            <div key={board._id} style={{ border: '1px solid #ccc', margin: '20px 0', padding: '15px', backgroundColor: '#f5f5f5' }}>
                                ...
                                <div style={{ display: 'flex', gap: '20px', overflowX: 'auto', paddingBottom: '10px' }}>
                                    {board.columns?.map(col => (
                                        <div key={col._id}
                                            // COLUMN DROP ZONE: Dropping here puts the task at the very bottom
                                            onDragOver={handleDragOver}
                                            onDrop={(e) => handleDrop(e, dash._id, board._id, col._id, col.tasks ? col.tasks.length : 0)}
                                            style={{ minWidth: '250px', backgroundColor: '#fff', padding: '10px', borderRadius: '6px', border: '1px solid #ddd' }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '10px' }}>
                                                {col.tasks?.map(task => (
                                                    <div key={task._id}
                                                        // MAKE TASK DRAGGABLE
                                                        draggable
                                                        onDragStart={(e) => handleDragStart(e, dash._id, board._id, col._id, task._id)}
                                                        // TASK DROP ZONE: Dropping on another task inserts it at this exact index
                                                        onDragOver={handleDragOver}
                                                        onDrop={(e) => handleDrop(e, dash._id, board._id, col._id, index)}
                                                        style={{ backgroundColor: '#e9ecef', padding: '8px', borderRadius: '4px', fontSize: '14px' }}>
